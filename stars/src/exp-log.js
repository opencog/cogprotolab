        ...
        var parse = function (rules, text) {
            ...
            function parse (start) {
                ...
                function getTerminal(offset, term) {
                    if (isTerminal (term)) {
                        ...
                        if (type !== '<' && type !== '/')
                            try {
                                return parseString (text, offset, JSON.parse('"' + term + '"'));
                            ...
                            return parseRegExp (text, offset, term.substring(1, term.length - 1));
                        ...
                        if (chart[i]) {
                            ...
                            for (j = 0; j < column.length; j++) {
                                ...
                                for (k = 0; k < rules.length; k++) {
                                    if (item.sequence[item.index] === rules[k][0][0])
                                        mergeItem (i, rules[k][1], 0, item, null);
                                }
                            }
                        }
                    }
            ...
            function makeParseTree (eof) {
                function isParent (item, parent, rec, fst) {
                    if (rec.indexOf (item) > -1)
                        return false;
                    
                    rec.push(item);
                    
                    if (item.sequence === parent.sequence && item.index === parent.index)
                        return true;
                ...
                function makeParseTree (eof) {
                    ...
                    while (parents.length > 0) {
                        if (item.index > 0) {
                            ...
                            if (item.sequence === reachParent.sequence && item.index === reachParent.index)
                                reachParent = {sequence: parents[parents.length - 1].sequence, index: parents[parents.length - 1].index};
                ...
                function getExpected (chart) {
                    ...
                    for (i = 0; i < end.length; i++) {
                        if (Array.isArray (end[i].sequence)) {
                            ...
                            else if(isString(str) && str.substr(0, 1) !== '<')
